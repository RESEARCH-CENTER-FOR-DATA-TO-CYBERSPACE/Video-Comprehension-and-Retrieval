...
def query(host, port, model_colomn, query_text):
    ...
    link0 = "http://127.0.0.1:5000/?video="+str(tmp[0]['videoID'][15:16+tmp[0]['videoID'][15:].find('_')])+'.mp4'+'&time='+str(int(tmp[0]['docID'][0:2])*60*60+int(tmp[0]['docID'][3:5])*60+int(tmp[0]['docID'][6:8]))
    link1 = "http://127.0.0.1:5000/?video="+str(tmp[1]['videoID'][15:16+tmp[1]['videoID'][15:].find('_')])+'.mp4'+'&time='+str(int(tmp[1]['docID'][0:2])*60*60+int(tmp[1]['docID'][3:5])*60+int(tmp[1]['docID'][6:8]))
    link2 = "http://127.0.0.1:5000/?video="+str(tmp[2]['videoID'][15:16+tmp[2]['videoID'][15:].find('_')])+'.mp4'+'&time='+str(int(tmp[2]['docID'][0:2])*60*60+int(tmp[2]['docID'][3:5])*60+int(tmp[2]['docID'][6:8]))

    return '您检索的信息存在于视频"'+tmp[0]['videoID']+'"，且其时间为：'+tmp[0]['docID'][0:2]+'小时'+''+tmp[0]['docID'][3:5]+'分'+tmp[0]['docID'][6:8]+'秒'+tmp[0]['docID'][-3:]+'毫秒'+'\n'\
        +'视频链接：http://127.0.0.1:5000/?video='+str(tmp[0]['videoID'][15:16+tmp[0]['videoID'][15:].find('_')])+'.mp4'+'&time='+str(int(tmp[0]['docID'][0:2])*60*60+int(tmp[0]['docID'][3:5])*60+int(tmp[0]['docID'][6:8]))\
        ...
        +'视频链接：http://127.0.0.1:5000/?video='+str(tmp[1]['videoID'][15:16+tmp[1]['videoID'][15:].find('_')])+'.mp4'+'&time='+str(int(tmp[1]['docID'][0:2])*60*60+int(tmp[1]['docID'][3:5])*60+int(tmp[1]['docID'][6:8]))\
        ...
        +'视频链接：http://127.0.0.1:5000/?video='+str(tmp[2]['videoID'][15:16+tmp[2]['videoID'][15:].find('_')])+'.mp4'+'&time='+str(int(tmp[2]['docID'][0:2])*60*60+int(tmp[2]['docID'][3:5])*60+int(tmp[2]['docID'][6:8]))\
...
model_colomn = gr.Dropdown(["video_gpt4o", "video_claude3dot5"], label="大模型选项")
