...
def read_and_extract_srt(file_path, start_time_str='00:00:00,000', end_time_str='100:00:00,000', interval_seconds=60, padding_seconds=5):
...
def encode_text(image2text_path, audio2text_path, output_path, mdoel_name='uer/sbert-base-chinese-nli'): 
    # 加载预训练的模型和分词器
    model = SentenceTransformer(mdoel_name, trust_remote_code=True)
    model.max_seq_length = 512
    df = pd.DataFrame(columns=['time', 'image_text', 'image_emb', 'audio_text', 'audio_emb'])
    data2 = None
    ...
    with open(image2text_path, 'r', encoding='utf-8') as f1:
        ...
        for time, text1 in data.items():
            ...
            if data2:
                ...
                df = pd.concat([df, pd.DataFrame({'time': time, 'image_text': text1, 'image_emb': [embedding1], 'audio_text': text2, 'audio_emb': [embedding2]})], ignore_index=True)
            else:
                df = pd.concat([df, pd.DataFrame({'time': time, 'image_text': text1, 'image_emb': [embedding1]})], ignore_index=True)
...
def convert_img2text_raw(image_path):
    ...
    for image in images:
        ...
        messages=[
            ...
            "role": "user",
            "content": [
                {"type": "text", "text": "What’s in this image? Make sure you use the same language as the image."},
                {
                "type": "image_url",
                "image_url": {
...
def convert_img2text(clips_path):
    ...
    for clip in clips:
        ...
        for image in images:
            # Getting the base64 string
            base64_image = encode_image(image)
            messages[1]['content'].append({"type": "image_url", "image_url": { "url": f"data:image/jpeg;base64,{base64_image}", "detail": "low"}})
        # messages = [
        #     {
        #         "role": "user",
        #         "content": [
        #             "These are frames from a video that I want to upload. Generate a compelling description that I can upload along with the video.",
        #             *map(lambda x: {"image": encode_image(os.path.join(image_path, x)), "resize": 768}, images[::]),
        #         ],
        #     },
        # ]
        response = client.chat.completions.create(
...
def speech2text(video, mdoel_path):
...
if __name__ == "__main__":
    ...
    logging.info('开始视频处理')
    video_id = speech2text(video_path, speech_recognition_model) # 'd7b24747-fade-4765-91e4-40ce04d7865d'
    ...
    encode_text(image2text_path, audio2text_path, embedding_path, embedding_model)
